...
function [xm,fv] = SCDE_DFPSO(fitness,N,P,c11,c12,w11,D,func_num,runs,MaxDT,Max_FES,range)
format long;  
flg=1;
...
%         popold=pop;
   ...
   while time>300
    ...
    %根据从众阈值挑选出跟随粒子，剩余的都归为独立粒子
        for i=2:m
            if NEXTpublic(i-1,3*D+2)==NEXTpublic(i,3*D+2)&&k<30
               sum1=sum1+NEXTpublic(i,3*D+2);
               k=k+1;
            elseif sum1>NEXTpublic(i,3*D+2)&&k<30
                sum1=sum1+NEXTpublic(i,3*D+2);
                k=k+1;
            else
                break;
    ...
    Pbest=public(1,2*D+1:3*D);
       ...
       if TIME==1
           Pg(func_num)=Gbest_val-y(func_num);
       end
        if fitcount>=Max_FES
            break;
        end
