...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [trend,S_bestval,iteration] = deopt(fname,I_NP,F_weight,F_CR,I_D,I_itermax,I_strategy,func_num,runs,Max_FES,range) 
...
% F_CR         = S_struct.F_CR;
% I_D          = S_struct.I_D;
FVr_minbound = range(1);
FVr_maxbound = range(2);
I_bnd_constr = 1;
 y=(-1400:100:1400);
% load fbias_data
% y=f_bias;
...
for time=1:runs
    ...
    %-----Select which vectors are allowed to enter the new population------------
          ...
          if (I_bnd_constr == 1)
             for j=1:I_D %----boundary constraints via bounce back-------
                if (FM_ui(k,j) > FVr_maxbound)
                   FM_ui(k,j) = FVr_maxbound + rand*(FM_origin(k,j) - FVr_maxbound);
                end
                if (FM_ui(k,j) < FVr_minbound)
                   FM_ui(k,j) = FVr_minbound + rand*(FM_origin(k,j) - FVr_minbound);
                end   
             end
          end
          %=====End boundary constraints==========================================

          S_tempval = feval(fname,FM_ui(k,:)',func_num);   % check cost of competitor
          I_nfeval  = I_nfeval + 1;
      ...
      if time==1
           Pg(1,fitold:I_nfeval)=S_bestval-y(func_num);
      end
      fitold=I_nfeval;
      gbest_val=S_bestval;
      I_iter = I_iter + 1;
